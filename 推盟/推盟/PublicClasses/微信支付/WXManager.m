...
#define WECHAT_MerchantAccount @"1319234701"
...
-(NSString *)genPackage:(NSMutableDictionary*)packageParams
{
    ...
    //生成签名
    sign        = [[self createMd5Sign:packageParams] uppercaseString];
    [packageParams removeObjectForKey:@"key"];
    //生成xml的package
...
-(NSString *)sendPrepay:(NSMutableDictionary *)prePayParams
{
    ...
    XMLTools *xml  = [XMLTools alloc];
    
    //开始解析
    [xml startParse:res];
    
    NSMutableDictionary *resParams = [xml getDict];
    NSLog(@"----%@-----  %@",resParams,resParams[@"return_msg"]);
    //判断返回
...
//    NSString *orderNO   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: @"1add1a30ac87aa2db72f57a2375d8fec"       forKey:@"nonce_str"];   //随机串
    [packageParams setObject: orderType                                 forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: orderName                                 forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: WECHAT_CALLBACK_URL                       forKey:@"notify_url"];  //支付结果异步通知
    ...
    [packageParams setObject:self.spKey                                 forKey:@"key"];
