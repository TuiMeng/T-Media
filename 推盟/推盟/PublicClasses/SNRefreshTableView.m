...
{
    UIView * aView;
    if (_refreshDelegate && [_refreshDelegate respondsToSelector:@selector(viewForFooterInSection:)]) {
        aView = [_refreshDelegate viewForFooterInSection:section];
    }
    return aView;
}
...
{
    CGFloat aHeight = 0.0;
    if (_refreshDelegate && [_refreshDelegate respondsToSelector:@selector(heightForFooterInSection:)]) {
        aHeight = [_refreshDelegate heightForFooterInSection:section];
    }
    return aHeight;
}
...
{
    if (!_normalLabel) {
        _normalLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 8 + (TABLEFOOTER_HEIGHT - 40)/2.0, DEVICE_WIDTH, 20)];
